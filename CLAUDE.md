...
`com.stratio.connectors:mysql-jdbc-<version>`). Single-module Maven (`jar`). CI builds on
...
- **The version-in-the-class-name is the whole point.** `com.mysql` is relocated to a package carrying
  the driver version, which is what lets two MySQL drivers load side by side. Do not "simplify" the
  relocation away — that is the feature.
...
- `dependency-reduced-pom.xml` is generated by the shade plugin — never edit it by hand.
- Consumers: the `sscc-mysql` connector. A change here can break it, and nothing in this repo's CI will
  tell you.
